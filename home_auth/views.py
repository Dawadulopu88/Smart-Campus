...
def signup_view(request):
    if request.method == "POST":
        ...
        profile_image = request.FILES.get("profile_image")  # Get uploaded profile image
        ...
        user.save()  # Save the user with the assigned role and profile image
        login(request, user)
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # Redirect user based on their role to appropriate dashboard
            if user.is_admin:
                return redirect("index")  # Admin goes to main dashboard
            elif user.is_teacher:
                return redirect("teacher_dashboard")  # Teacher goes to teacher dashboard
            elif user.is_student:
                return redirect("student_dashboard")  # Student goes to student dashboard
            else:
                messages.error(request, "Invalid user role")
                return redirect("login")  # Redirect to login in case of error
